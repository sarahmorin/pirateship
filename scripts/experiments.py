...
class Experiment:
    ...
    def remote_build(self):
        ...
        try:
            ...
            print("Failed to create repo directory. Continuing...")
        ...
        cmds.append(
            f"cd {remote_repo} && source $HOME/.cargo/env && {self.build_command}"
        )
...
"""
    ...
    def deploy(self, deployment: Deployment, last_git_hash="", last_git_diff="", last_build_command=""):
        ...
        # CloudLab uses /users instead of /home
        self.remote_workdir = f"/users/{deployment.ssh_user}/{deployment.workdir}/experiments/{self.name}"
