...
class AutobahnExperiment(Experiment):
    def copy_back_build_files(self):
        ...
        remote_repo = f"/users/{self.dev_ssh_user}/repo/autobahn"
    ...
    def setup_ssh_key_on_dev_vm(self):
        """Copy the local SSH key to the dev VM so it can SSH to other nodes"""
        remote_ssh_dir = f"/users/{self.dev_ssh_user}/.ssh"
        remote_key_path = f"{remote_ssh_dir}/id_ed25519"
        
        # Check if key already exists on dev VM
        check_result = run_remote_public_ip([
            f"test -f {remote_key_path} && echo 'exists' || echo 'missing'"
        ], self.dev_ssh_user, self.dev_ssh_key, self.dev_vm, hide=True)
        
        if "exists" in check_result[0]:
            print("SSH key already exists on dev VM")
            return
        
        print("Copying SSH key to dev VM for inter-node communication...")
        # Ensure .ssh directory exists with correct permissions
        run_remote_public_ip([
            f"mkdir -p {remote_ssh_dir}",
            f"chmod 700 {remote_ssh_dir}"
        ], self.dev_ssh_user, self.dev_ssh_key, self.dev_vm, hide=True)
        
        # Copy the SSH private key to dev VM
        ...
        print("SSH key setup complete on dev VM")
    ...
    def generate_arbiter_script(self):
        # Setup SSH key on dev VM for inter-node communication
        self.setup_ssh_key_on_dev_vm()
        
        # The arbiter script runs on the remote dev VM, so we need the remote SSH key path
        # CloudLab uses /users/<username>/ not /home/<username>/
...
SCP_CMD="rsync -avz -e 'ssh -o StrictHostKeyChecking=no -i {remote_ssh_key}'"
