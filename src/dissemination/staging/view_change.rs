...
use crate::{
    ...
    utils::{deserialize_proto_block, get_parent_hash_in_proto_block_ser},
};
...
impl Staging {
    ...
    pub(super) async fn update_view(&mut self, view_num: u64, config_num: u64) {
        ...
        let (payload, vc_msg) = {
            // Jumping through hoops to avoid cloning.
            ...
            if let Some(crate::proto::rpc::proto_payload::Message::ViewChange(vc_msg)) =
                payload.message
            {
                (msg, vc_msg)
            } else {
        ...
        self.fork_receiver_command_tx
            ...
            .unwrap();
        self.pacemaker_tx
            .send(PacemakerCommand::MyViewJumped(
                self.view,
                self.config_num,
                vc_msg,
            ))
    ...
    pub(super) async fn maybe_stabilize_view(&mut self, qc: &ProtoQuorumCertificate) {
        ...
        self.client_reply_tx
            .send(ClientReplyCommand::StopCancelling)
            .await
            .unwrap();
    }
    ...
    async fn apply_fork_choice_rule(
        ...
        let mut applicable_forks = self
            .fork_choice_filter_from_forks(forks, &mut fork_stats)
            .await;
        assert!(applicable_forks.len() > 0);
        let chosen_fork = applicable_forks.remove(0);
        let chosen_fork_stat = self
            .send_blocks_to_broadcaster_and_get_stats(chosen_fork)
            .await;
