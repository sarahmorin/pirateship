...
/// The BlockSequencer is responsible for sequencing batches into blocks.
/// It receives batches from the BatchProposer, and QCs from the QuorumCertifier.
/// It then creates blocks and sends them to the BlockBroadcaster and replies to clients.
...
impl BlockSequencer {
    ...
    // Run the BlockSequencer with the worker loop.
    ...
    /// Check if I am the leader in the current view.
    ...
    // The main worker loop for the BlockSequencer.
    // It listens for new batches, QCs, and control commands.
    // - If view is stable and I am the leader, I listen for new batches to propose. (and QCs and control commands)
    // - Otherwise, I only listen for QCs and control commands.
    ...
    // Handle a new batch to be sequenced into a block.
    // Prepare with crypto service and send to BlockBroadcaster and ClientReply.
    // QUESTION: Do we need the signing configs for dissemination?
    async fn handle_new_batch(
        ...
        // TODO: We probably want to change this to a CAR block
        //      and then in consensus we should be sending tip cuts instead of batches
        ...
        // Prepare block from crypto service
        ...
        // Send hash and replies to client
        self.client_reply_tx
        ...
        // Send block to block BlockBroadcaster
    ...
    // Add new QCs to the current QC list.
    ...
    // Handle control commands for view changes.
    // - NewUnstableView: Change to a new unstable view.
    // - ViewStabilised: Change to a stable view.
    // - NewViewMessage: Change to a new unstable view given a new view message
