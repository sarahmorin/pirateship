...
/// The BlockBroadcaster is responsible for storing and forwarding blocks to other nodes,
/// logserver and staging area.
/// It receives blocks that are ready to be broadcasted from the BlockSequencer.
...
impl BlockBroadcaster {
    ...
    /// Run the BlockBroadcaster worker loop.
    ...
    /// The main worker loop for the BlockBroadcaster.
    ...
    /// Get the list of all nodes except myself.
    ...
    /// - UpdateCI: Update the commit index.
    /// - NextAEForkPrefix: Add blocks to the fork prefix buffer.
    ...
    /// Store the block and forward it internally to logserver and staging area.
    ...
    /// Process a block created by myself that needs to be broadcast.
    ...
    /// Get the threshold for byzantine broadcast.
    /// NOTE: We will need something similar or the u+1 dissemination threshold
    ...
    /// Process a block received from other nodes.
    /// No need to forward to other nodes, just store and logserver/staging.
    ...
    /// Maybe act evil by equivocating the AE fork to half of the nodes.
    ...
    /// Broadcast an AE fork to a list of nodes as ProtoAppendEntries.
