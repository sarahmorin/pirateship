/// Dissmination module: Batch proposer
/// Based on copy of consensus module: Batch proposer
...
pub struct BatchProposer {
    ...
    // Receive incoming transactions from Clients
    batch_proposer_rx: Receiver<TxWithAckChanTag>,
    // Send to BlockSequencer (aka block maker)
...
impl BatchProposer {
    ...
    /// Run the batch proposer using the worker loop
    ...
    /// Register a new entry for performance tracking with probability approx 1/batch_size
    ...
    /// Add an event for performance tracking
    fn perf_add_event(&mut self, entry: usize, event: &str) {
        #[cfg(feature = "perf")]
        self.perf_counter.borrow_mut().new_event(event, &entry);
    }

    /// Add an event for all registered entries and deregister all entries
    ...
    /// Worker loop for the batch proposer
    /// Processes incoming transactions and proposes batches
    async fn worker(&mut self, work_counter: usize) -> Result<(), Error> {
        ...
        // QUESTION: Why are we checking this in every loop iteration? Does it change??
        let max_batch_size = self.config.get().consensus_config.max_backlog_batch_size;

        // Propose a new batch if either the batch size limit is reached
    ...
    // QUESTION: What is this? Is it just for evaluations?
