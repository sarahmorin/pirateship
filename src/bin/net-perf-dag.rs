...
impl ProfilerNode {
    ...
    pub fn run(node: Arc<Self>, payload_sz: usize) -> JoinSet<()> {
        ...
        js.spawn(async move {
            ...
            info!("{:?}", send_list);
            // DAG MODE: All nodes broadcast (not just node1)
            // This simulates parallel dissemination instead of leader-only broadcast
            loop {
        ...
        js.spawn(async move {
            ...
            loop {
                ...
                // DAG MODE: All nodes send AND receive
                let now_sent = node3.ctx.0.bytes_completed_bcasts.load(Ordering::SeqCst);
                let now_recv = node3.ctx.0.bytes_received_msgs.load(Ordering::SeqCst);

                let send_mbps = ((now_sent - last_sent) as f64) * 8.0 / (1024.0 * 1024.0);
                let recv_mbps = ((now_recv - last_recv) as f64) * 8.0 / (1024.0 * 1024.0);

                info!(
                    "Throughput - Send: {:.2} Mbps, Recv: {:.2} Mbps, Total: {:.2} Mbps",
                    send_mbps, recv_mbps, send_mbps + recv_mbps
                );
